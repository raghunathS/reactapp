...
interface AwsTicketsByMonthChartProps {
  data: EnvironmentSummaryResponse | null;
  loading: boolean;
}

const AwsTicketsByMonthChart: FC<AwsTicketsByMonthChartProps> = ({ data, loading }) => {

  if (loading) {
    return <Container><Header variant="h2">Loading AWS Tickets by Month...</Header></Container>;
  }

  const chartData = data?.aws;
...
export default AwsTicketsByMonthChart;
